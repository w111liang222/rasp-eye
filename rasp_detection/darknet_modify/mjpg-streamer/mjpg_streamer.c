...
Return Value:
******************************************************************************/
globals* mjpg_streamer_startup(void)
{
    int argc=5;
    ...
    for(int i = 0; i < global.incnt; i++) {
        ...
        if(pthread_mutex_init(&global.in[i].net, NULL) != 0) {
            LOG("could not initialize mutex variable\n");
            closelog();
            exit(EXIT_FAILURE);
        }
        ...
        if(pthread_cond_init(&global.in[i].net_update, NULL) != 0) {
            LOG("could not initialize condition variable\n");
            closelog();
            exit(EXIT_FAILURE);
        }

        size_t tmp = (size_t)(strchr(input[i], ' ') - input[i]);
        global.in[i].ipl_net   = NULL;
        global.in[i].stop      = 0;
        ...
        for (int j = 0; j<MAX_PLUGIN_ARGUMENTS; j++) {
    ...
    for(int i = 0; i < global.outcnt; i++) {
        size_t tmp = (size_t)(strchr(output[i], ' ') - output[i]);
